...
def test_home_page_renders_controls(page, app_url):
    ...
    expect(page.get_by_role("heading", name="Leeward")).to_be_visible()
    ...
    expect(page.get_by_role("button", name="Find leeward names")).to_be_visible()


@pytest.mark.e2e
def test_screen_returns_analysis_and_levels(page, app_url):
    page.goto(app_url, wait_until="networkidle")
    page.get_by_role("button", name="Find leeward names").click()
    expect(page.locator("#status-output")).to_contain_text("companies", timeout=45_000)
    expect(page.locator("#analysis-output")).to_contain_text("Overall grade")
